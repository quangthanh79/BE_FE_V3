...
router.post ('/get_user_info', async (req, res) => {
  ...
  try {
    ...
    data.is_friend = "NOT_FRIEND";
    if (tokenUser && user_id != tokenUser.id) {
      let friendOrNot = user.friends.findIndex(element => element.friend._id.equals(tokenUser.id));
      if(friendOrNot) {
      data.is_friend = "IS_FRIEND";
      } else {
        let senderOrNot = user.friendRequestSent.findIndex(element => element.friend._id.equals(tokenUser.id));
        if(senderOrNot) {
          data.is_friend = "REQUESTING";
        } else {
          let receiverOrNot = user.friendRequestReceived.findIndex(element => element.friend._id.equals(tokenUser.id));
          if(receiverOrNot) {
